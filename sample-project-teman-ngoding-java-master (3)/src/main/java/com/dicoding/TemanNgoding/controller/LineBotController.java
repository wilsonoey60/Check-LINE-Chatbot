...
public class LineBotController {
    ...
    private void handleGroupChats(String replyToken, String textMessage, String groupId) {
        ...
        if (msgText.contains("bot leave")) {
            if (sender == null) {
                botService.replyText(replyToken, "Hi, tambahkan dulu bot Belajar Mudah sebagai teman!");
            } else {
        ...
        } else if (msgText.contains("lihat daftar mata pelajaran")) {
            ...
            if(sender != null) {
                if (!sender.getDisplayName().isEmpty() && (lineId.length() > 0)) {
                    if (dbService.regLineID(sender.getUserId(), lineId, sender.getDisplayName()) != 0) {
                        showCarouselEvents(replyToken, "Pendaftaran user berhasil. Berikut daftar mata pelajaran yang bisa kamu ikuti:");
                    } else {
                        userNotFoundFallback(replyToken, "Gagal melakukan pendaftaran user! Ikuti petunjuk berikut ini:");
                    }
                } else {
                    userNotFoundFallback(replyToken, "User tidak terdeteksi. Tambahkan dulu bot Belajar Mudah sebagai teman!");
                }
            } else {
                userNotFoundFallback(replyToken, "Hi, tambahkan dulu bot Belajar Mudah sebagai teman!");
            }
        }
    }
    ...
    private void userNotFoundFallback(String replyToken, String additionalInfo) {
        ...
        messages.add("Aku akan mencarikan link aktif di chatbot Belajar Mudah! Tapi, kasih tahu dulu LINE ID kamu (pake \'id @\' ya)");
    ...
    private void broadcastNewFriendJoined(String eventId, String newFriendId) {
        ...
        String msg = "Hi, ada teman baru telah bergabung di bot kami " + eventId;
